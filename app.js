...
function extractTicker(title) {
  const skip = new Set(["THE","AND","FOR","FROM","WITH","AFTER","OVER","AMID","THAT","THIS","WILL","HAVE","BEEN","MORE","THAN","YEAR","RATE","CUTS","SAID","SAYS","WHY","HOW","WHAT","NEW","NOW","ITS","ALL","ARE","NOT","BUT","HAS","CAN","MAY","WAY","ONE","TWO","TOP","OIL","GAS","USD","GDP","IPO","ETF","CEO","CFO","SEC","FED","U.S","AGM","AI"]);
...
function App() {
  ...
  const handleAlertsToggle = async () => {
    ...
    if (notifStatus === "denied") {
      alert("התראות חסומות. לחץ על 🔒 ליד הכתובת ← הרשאות אתר ← התראות ← אפשר");
      return;
    }
    ...
    try {
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content: `You are a sharp financial analyst. Summarize these market headlines in 3-4 concise bullet points. Focus on key themes and actionable insights. Be direct.\n\nHeadlines:\n${headlines}` }]
        }),
      });
      const data = await resp.json();
  ...
  return React.createElement('div', { style: s({ minHeight:"100vh", background:"#080b12", color:"#e2e8f0", fontFamily:"'Inter',-apple-system,sans-serif" }) },

    // HEADER
    React.createElement('div', { style: s({ background:"rgba(8,11,18,0.97)", backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(255,255,255,0.06)", padding:"0 16px", position:"sticky", top:0, zIndex:100, height:56, display:"flex", alignItems:"center", justifyContent:"space-between" }) },
      React.createElement('div', { style: s({ display:"flex", alignItems:"center", gap:10 }) },
        React.createElement('div', { style: s({ width:32, height:32, borderRadius:8, background:"linear-gradient(135deg,#0ea5e9,#6366f1)", display:"flex", alignItems:"center", justifyContent:"center" }) },
          React.createElement('span', { style: s({ fontSize:13, fontWeight:900, color:"#fff" }) }, "MP")
        ),
        React.createElement('div', null,
          React.createElement('div', { style: s({ fontSize:15, fontWeight:700, color:"#f1f5f9", letterSpacing:"-0.3px" }) }, "MarketPulse"),
          React.createElement('div', { style: s({ fontSize:10, color:"#475569" }) }, "Live News Feed")
        )
      ),
      React.createElement('div', { style: s({ display:"flex", alignItems:"center", gap:8 }) },
        React.createElement('div', { style: s({ display:"flex", alignItems:"center", gap:5, background: marketOpen?"rgba(34,197,94,0.1)":"rgba(239,68,68,0.1)", border:`1px solid ${marketOpen?"rgba(34,197,94,0.25)":"rgba(239,68,68,0.25)"}`, borderRadius:20, padding:"3px 9px" }) },
          React.createElement('div', { style: s({ width:6, height:6, borderRadius:"50%", background: marketOpen?"#22c55e":"#ef4444", boxShadow: marketOpen?"0 0 6px #22c55e":"none" }) }),
          React.createElement('span', { style: s({ fontSize:10, fontWeight:600, color: marketOpen?"#22c55e":"#ef4444" }) }, marketOpen?"OPEN":"CLOSED")
        ),
        React.createElement('div', { style: s({ fontSize:11, color:"#334155", fontVariantNumeric:"tabular-nums" }) }, now.toLocaleTimeString("en-US",{hour12:false}))
      )
    ),

    // CONTENT
    React.createElement('div', { style: s({ maxWidth:640, margin:"0 auto", padding:"12px 12px 40px" }) },

      // CONTROLS
      React.createElement('div', { style: s({ display:"flex", gap:8, marginBottom:12, alignItems:"center", flexWrap:"wrap" }) },
        React.createElement('button', { onClick: isLive?stopLive:startLive, style: s({ display:"flex", alignItems:"center", gap:6, padding:"8px 16px", borderRadius:8, border:"none", cursor:"pointer", fontFamily:"inherit", fontSize:12, fontWeight:600, background: isLive?"rgba(239,68,68,0.12)":"linear-gradient(135deg,#0ea5e9,#6366f1)", color: isLive?"#ef4444":"#fff" }) },
          React.createElement('div', { style: s({ width:6, height:6, borderRadius:"50%", background: isLive?"#ef4444":"rgba(255,255,255,0.9)", animation: isLive&&!refreshing?"livePulse 1.5s infinite":"none" }) }),
          isLive ? (refreshing?"Fetching...":"Stop Live") : "Go Live"
        ),
        React.createElement('button', { onClick: handleAlertsToggle, style: s({ padding:"8px 12px", borderRadius:8, border:`1px solid ${alertBorder}`, cursor:"pointer", fontFamily:"inherit", fontSize:12, fontWeight:500, background:alertBg, color:alertColor }) }, alertLabel),
        React.createElement('button', { onClick: ()=>setSoundEnabled(s=>!s), style: s({ padding:"8px 12px", borderRadius:8, border:`1px solid ${soundEnabled?"rgba(251,191,36,0.3)":"rgba(255,255,255,0.08)"}`, cursor:"pointer", fontFamily:"inherit", fontSize:12, background: soundEnabled?"rgba(251,191,36,0.08)":"rgba(255,255,255,0.04)", color: soundEnabled?"#fbbf24":"#64748b" }) }, soundEnabled?"🔊":"🔇"),
        React.createElement('button', { onClick: getAiSummary, disabled: aiLoading, style: s({ padding:"8px 12px", borderRadius:8, border:"1px solid rgba(167,139,250,0.3)", cursor: aiLoading?"wait":"pointer", fontFamily:"inherit", fontSize:12, fontWeight:500, background:"rgba(167,139,250,0.08)", color:"#a78bfa", opacity: aiLoading?0.6:1 }) }, aiLoading?"✦ Analyzing...":"✦ AI Brief"),
        newCount > 0 && React.createElement('div', { style: s({ marginLeft:"auto", background:"rgba(251,146,60,0.1)", color:"#fb923c", border:"1px solid rgba(251,146,60,0.2)", borderRadius:20, padding:"4px 10px", fontSize:11, fontWeight:700 }) }, `+${newCount} new`)
      ),

      // ERROR
      fetchError && React.createElement('div', { style: s({ background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.2)", borderRadius:8, padding:"10px 14px", marginBottom:12, fontSize:12, color:"#f87171" }) }, "⚠ " + fetchError),

      // AI SUMMARY
      aiSummary && React.createElement('div', { style: s({ background:"rgba(167,139,250,0.05)", border:"1px solid rgba(167,139,250,0.12)", borderRadius:12, padding:"14px 16px", marginBottom:12 }) },
        React.createElement('div', { style: s({ display:"flex", alignItems:"center", gap:6, marginBottom:10 }) },
          React.createElement('span', { style: s({ fontSize:11, fontWeight:600, color:"#a78bfa" }) }, "✦ AI Market Brief"),
          React.createElement('button', { onClick:()=>setAiSummary(""), style: s({ marginLeft:"auto", background:"none", border:"none", color:"#475569", cursor:"pointer", fontSize:16, padding:0 }) }, "×")
        ),
        React.createElement('div', { style: s({ fontSize:12, lineHeight:1.75, color:"#cbd5e1", whiteSpace:"pre-wrap" }) }, aiSummary)
      ),

      // FILTERS
      React.createElement('div', { style: s({ display:"flex", gap:5, flexWrap:"wrap", marginBottom:12, alignItems:"center" }) },
        React.createElement('button', { onClick:()=>setFilter("all"), style: s({ padding:"5px 11px", borderRadius:6, border:`1px solid ${filter==="all"?"rgba(255,255,255,0.2)":"rgba(255,255,255,0.06)"}`, background:filter==="all"?"rgba(255,255,255,0.1)":"transparent", color:filter==="all"?"#f1f5f9":"#475569", fontSize:11, fontWeight:filter==="all"?700:400, cursor:"pointer", fontFamily:"inherit" }) }, "All"),
        ...Object.entries(CATEGORY_CONFIG).map(([cat,c]) =>
          React.createElement('button', { key:cat, onClick:()=>setFilter(cat), style: s({ padding:"5px 10px", borderRadius:6, border:`1px solid ${filter===cat?c.border:"rgba(255,255,255,0.06)"}`, background:filter===cat?c.bg:"transparent", color:filter===cat?c.color:"#475569", fontSize:11, fontWeight:filter===cat?700:400, cursor:"pointer", fontFamily:"inherit", transition:"all 0.15s" }) }, c.icon+" "+c.label)
        ),
        React.createElement('div', { style: s({ marginLeft:"auto", fontSize:10, color:"#334155" }) },
          filtered.length + " · " + lastUpdate.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})
        )
      ),

      // NEWS LIST
      React.createElement('div', { style: s({ display:"flex", flexDirection:"column", gap:2 }) },
        filtered.length === 0 && React.createElement('div', { style: s({ textAlign:"center", padding:"48px 0", color:"#334155" }) },
          React.createElement('div', { style: s({ fontSize:24, marginBottom:8 }) }, "◇"),
          React.createElement('div', { style: s({ fontSize:13 }) }, "No news in this category")
        ),
        filtered.map(item => {
          ...
          return React.createElement('div', {
            ...
            React.createElement('div', { style: s({ display:"flex", alignItems:"center", gap:6, marginBottom:7 }) },
              React.createElement('span', { style: s({ fontSize:9, fontWeight:700, letterSpacing:"0.5px", color: isMock?"#334155":cat.color, background: isMock?"rgba(255,255,255,0.03)":cat.bg, border:`1px solid ${isMock?"rgba(255,255,255,0.06)":cat.border}`, padding:"2px 7px", borderRadius:4 }) }, cat.icon+" "+cat.label),
              item.ticker && React.createElement('span', { style: s({ fontSize:10, fontWeight:700, color: isMock?"#334155":"#94a3b8", background:"rgba(148,163,184,0.08)", border:"1px solid rgba(148,163,184,0.1)", padding:"2px 7px", borderRadius:4 }) }, item.ticker),
              isFlashing && React.createElement('span', { style: s({ fontSize:9, fontWeight:700, color:"#0ea5e9", background:"rgba(14,165,233,0.1)", border:"1px solid rgba(14,165,234,0.25)", padding:"2px 7px", borderRadius:4, marginLeft:"auto" }) }, "NEW"),
              isMock && React.createElement('span', { style: s({ fontSize:9, color:"#1e293b", marginLeft:"auto" }) }, "example")
            ),
            React.createElement('div', { style: s({ fontSize:13, lineHeight:1.55, color: isMock?"#334155":"#e2e8f0", fontWeight:450, marginBottom:8 }) }, item.title),
            React.createElement('div', { style: s({ display:"flex", alignItems:"center", gap:8 }) },
              React.createElement('span', { style: s({ fontSize:10, fontWeight:600, color: isMock?"#334155":srcColor }) }, item.source),
              React.createElement('span', { style: s({ fontSize:10, color:"#1e293b" }) }, "·"),
              React.createElement('span', { style: s({ fontSize:10, color: isMock?"#1e293b":"#475569" }) }, timeAgo(item.time)),
              !isMock && item.url && item.url !== "#" && React.createElement('span', { style: s({ marginLeft:"auto", fontSize:11, color:"#334155" }) }, "↗")
            )
          );
        })
      ),

      React.createElement('div', { style: s({ marginTop:32, textAlign:"center", color:"#1e293b", fontSize:10 }) },
